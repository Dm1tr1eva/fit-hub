...
const macroDetails = computed(() => {
  ...
  return [
    { short: "P", icon: "i-lucide-beef", current: store.totals.protein, goal: store.dailyProtein, color: "bg-blue-500" },
    { short: "F", icon: "i-lucide-hamburger", current: store.totals.fat, goal: store.dailyFat, color: "bg-yellow-500" },
    { short: "C", icon: "i-lucide-croissant", current: store.totals.carb, goal: store.dailyCarb, color: "bg-green-500" },
  ]
})
</script>
    ...
    <div class="flex items-center gap-4 rounded-2xl bg-white p-4 shadow-sm">
      <div class="relative h-32 w-32 shrink-0">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center text-center">
          <p class="text-2xl font-bold leading-none">{{ remaining }}</p>
      ...
      <div class="min-w-0 flex-1 space-y-2">
        ...
        >
          ...
          <span class="w-3 shrink-0 text-sm font-semibold">{{ macro.short }}</span>
          <div class="h-2 min-w-0 flex-1 rounded-full bg-gray-100">
            <div
